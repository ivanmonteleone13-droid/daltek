...
export default function Hero() {
  ...
  return (
    <section className="relative overflow-hidden bg-gradient-to-br from-[#1a5f7a] via-[#14a3d7] to-[#0cb9e9]/80 text-white">
      {heroImage && (
        <>
          <img
            src={heroImage}
            alt={`${business.name} — serviceverkstad på ${business.address.street}, ${business.address.city}`}
            className="absolute inset-0 h-full w-full object-cover"
            fetchPriority="high"
          />
          <div className="absolute inset-0 bg-[#1a5f7a]/80" aria-hidden />
        </>
      )}
      ...
      <div className="relative mx-auto grid max-w-6xl gap-10 px-4 py-20 sm:px-6 lg:grid-cols-2 lg:items-center lg:py-28">
        <div>
          ...
          <h1 className="text-4xl font-bold leading-tight tracking-tight sm:text-5xl lg:text-6xl">
            Elsparkcyklar
            <span className="block text-[#edf7ef]">service på Molngatan</span>
          </h1>
          <p className="mt-6 max-w-xl text-lg leading-relaxed text-white/90">{business.description}</p>
          <div className="mt-8 flex flex-col gap-4 sm:flex-row">
            <a
              href={business.bookingUrl}
              className="inline-flex items-center justify-center rounded-full bg-white px-8 py-4 text-base font-semibold text-[#1a5f7a] shadow-lg transition hover:opacity-90"
            >
